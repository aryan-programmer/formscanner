...
import com.albertoborsetta.formscanner.ImageDrawer;
...
import com.albertoborsetta.formscanner.gui.*;
...
public class FormScannerModel {
	...
	public FormScannerModel() throws UnsupportedEncodingException {
		...
		if(".jar".equals(path.substring(path.length() - 4))) {
		...
		historyGroupNameTemplate    = new ArrayList<String>(Arrays.asList(
			StringUtils.split(configurations.getProperty(
		...
		if(formTemplate != null) {
			...
			case ANALYZE_FILES_ALL:
				if(!openedFiles.isEmpty()) {
					...
					String           tmstmp = sdf.format(today);
					view.setRenameControllersEnabled(false);
					view.setScanControllersEnabled(false);
					view.setScanAllControllersEnabled(false);
					view.setScanCurrentControllersEnabled(false);
					...
					for(Entry<Integer, File> openedFile : openedFiles.entrySet()) {
						...
						try {
							...
							BufferedImage imgRes = ImageDrawer.getParsedImage(image, filledForm, this);
							ImageDrawer.saveImageRelativeTo(imageFile, imgRes, tmstmp);
						} catch(Exception e) {
							logger.debug("Error", e);
						}
	...
	public void linkToHelp(URL url) {
		...
		try {
			...
			if(osName.indexOf("win") >= 0) {
				...
				// Linux/Unix
				String browsers[] = {
					"firefox", "opera", "konqueror", "epiphany", "mozilla", "netscape", "safari", "links", "lynx"
				};
				String browser = null;
...
//				historyBarcodeNameTemplate.get(0);
	...
	public void setLastPosition(Frame frame, Rectangle position) {
		...
		String[] positions = {
			String.valueOf(position.x), String.valueOf(position.y), String.valueOf(position.width),
