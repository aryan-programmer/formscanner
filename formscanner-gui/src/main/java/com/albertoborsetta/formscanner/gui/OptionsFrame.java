...
import com.albertoborsetta.formscanner.gui.builder.*;
...
public class OptionsFrame extends InternalFrame implements TabbedView {
	...
	private void setDefaultValues() {
		...
		resetAutoNumberingQuestions.setSelected(model.isResetAutoNumberingQuestions());
	...
	public int getCropFromTop() {
		return (Integer) cropFromTop.getValue();
	}

	public int getCropFromLeft() {
		return (Integer) cropFromLeft.getValue();
	}

	public int getCropFromRight() {
		return (Integer) cropFromRight.getValue();
	}

	public int getCropFromBottom() {
		return (Integer) cropFromBottom.getValue();
	}
	...
	private boolean verifyTemplatePanel() {
		return (
			(groupsEnabled.isSelected() ^ model.isGroupsEnabled()) || (
				                                                          resetAutoNumberingQuestions
					                                                          .isEnabled()
			                                                          ) ^ model.isResetAutoNumberingQuestions() || model
				.getHistoryNameTemplate(FormScannerConstants.GROUP)
	...
	private boolean verifyGuiPanel() {
		return (
			       !((String) fontTypeComboBox.getSelectedItem()).equals(model
				                                                             .getFontType())
		       ) || (
			       (Integer) fontSizeComboBox
				       .getSelectedItem() != model.getFontSize()
		       ) || (
			       !(
				       (String) lookAndFeelComboBox
					       .getSelectedItem()
			       ).equals(model.getLookAndFeel())
		       );
	}
	...
	private boolean verifyScanPanel() {
		return ((Integer) thresholdValue.getValue() != model.getThreshold()) || (
			(Integer) densityValue
				.getValue() != model.getDensity()
		) || (
			       (Integer) shapeSizeValue
				       .getValue() != model.getShapeSize()
		       ) || (
			       shapeTypeComboBox
				       .getSelectedIndex() != model.getShapeType().getIndex()
		       ) || (
			       cornerTypeComboBox
